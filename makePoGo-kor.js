		...
		*/
		...
		function editColors() {
			const selectRow = `<p class='pogo-colors'>{{title}}<br>
				색상: <input type='color' id='{{id}}Color'> 투명도: <select id='{{id}}Opacity'>
			...
			const html =
				...
				selectRow.replace('{{title}}', '셀 색상 선택2').replace(`{{width}}`, ` 굵기: <input type='number' min='1' max='8' id='{{id}}Width' size='2'> `).replace(/{{id}}/g, 'grid1') +
				...
				selectRow.replace('{{title}}', `${poiCellLevel}셀에서 스탑이나 체육관이 있는 셀`).replace(/{{id}}/g, 'cell17Filled').replace(`{{width}}`, '') +
				selectRow.replace('{{title}}', `${gymCellLevel}셀에서 체육관 3개 생성된 셀`).replace(/{{id}}/g, 'cell14Filled').replace(`{{width}}`, '') +
			...
			const updatedSetting = function (id) {
				saveSettings();
				if (id == 'nearbyCircleBorder' || id == 'nearbyCircleFill') {v
					redrawNearbyCircles();
				} else {
					updateMapGrid();
				}
				...
				if (!seenCells[cellStr]) {
					...
					if (isCellOnScreen(bounds, cell)) {
						...
						if (cellData) {
                            ...
                            const totalStops = computeTotalStops(cellData); // 추가된 함수
							switch (missingStops) {
								case 0:
									if (missingGyms <= 0) {
        ...
        //추가
        function computeTotalStops(cellData){
